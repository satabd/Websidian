...
| A12 | 🟡 2026-09-23: the self-contained package works (`npm run pack:openclaw`, `plugins install npm-pack:…`, runtime provisioned on first start — [[OpenClaw plugin#One command: the package]]). Still open: **publish** it (npm scope or ClawHub) so it installs by name, updates with `openclaw plugins update`, and appears in the Control UI's plugin browser; try the package in Docker (`alpine/openclaw`) and on Linux — [[OpenClaw plugin#Ways to distribute it]] |
