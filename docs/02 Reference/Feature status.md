...
| Agent panel for readers (`"agents": "readers"`, `<site>/_ask/`): Review only, for users signed in by the trusted proxy — used beside notes in the Hermes tab (Hermes first, Claude Code, Codex) | 🟡 | Browser-tested 2026-09-23 in a throw-away Hermes dashboard: real Claude Code and Hermes turns, a follow-up from the same conversation, on a read-only vault; server tests for the door. **Deployed to `hermes01` 2026-09-23** (all three agents found, Review only, OAuth only for Claude Code and Codex); a live answer there waits on sign-ins and quota — [[Known issues]], [[Hermes plugin#Asking an agent]] |
| Shell mode: host palette (`websidian:palette`) and `&flow=1` (the page is as tall as its content and scrolls with the host) | ✅ | Browser-tested in the Hermes tab 2026-09-23: palette on notes, graph; flow on notes and the tree, links keep `flow=1`, heights follow the note — [[Embedding in your website#Inside another application shell mode]] |
...
| Hermes dashboard tab, redesigned: native Overview / Memory (entries + meter) / Skills (by category) / Browse / Graph, one search over every vault, a reading pane that flows with the dashboard page, a reading-theme picker (Match Hermes, Light, Dark, Paper, Nord, Night) — [[Hermes plugin#Dashboard tab]] | 🟡 browser-tested 2026-09-23 in a throw-away Hermes dashboard (the real Hermes SPA and plugin loader) on this vault, Hermes's bundled skills and sample memory; **deployed to `hermes01` 2026-09-23** and checked there in the signed-in browser on the real vaults (memory cards 87 % / 97 % full) |
...
| OpenClaw plugin: **on ClawHub** as `websidian` (`openclaw plugins install clawhub:websidian`) — [[OpenClaw plugin#Installing it for someone else]] | ✅ 2026-09-23, `websidian@0.2.0` published (package and release scans clean, source-linked), installed by name into a fresh OpenClaw 2026.9.5 Gateway and serving the workspace |
| OpenClaw plugin: **self-deployable package** (`npm run pack:openclaw` → one tarball; `openclaw plugins install npm-pack:…`; the plugin installs its bundled runtime on first start; no config serves the default workspace) — [[OpenClaw plugin#One command: the package]] | ✅ 2026-09-23, installed into a fresh OpenClaw 2026.9.5 Gateway (Windows, portable Node 26): runtime provisioned in 13 s, `inspect` clean, notes and vendor files proxied, Memory page and reading pane browser-tested. Not yet tried in Docker or on macOS/Linux |
