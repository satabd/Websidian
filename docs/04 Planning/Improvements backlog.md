...
| A9 | 🟡 deployed 2026-09-17 ([[OpenClaw plugin]] in `clawat02`, workspace vault, pages on port 18794). `clawat02` has since moved to OpenClaw 2026.9.5 with the old plugin: **redeploy** (container installer, capability consent, `allowConversationAccess`, Custom plugin UI) to get the Memory page there. Still open: one real chat turn — the *Notes updated:* footer and the `SOUL.md` approval prompt are unit-tested contracts only; blocked by the OpenAI backend (`400` with the plugin enabled and disabled alike; weekly quota at 0 %, resets ~2026-09-19) |
...
| A12 | OpenClaw plugin: `openclaw plugins install` from a package (npm/ClawHub) — today the plugin folder is linked or listed in `plugins.load.paths`; the runtime copy still needs the installer. The plugin alone now packs complete (`dist/` added to `files`, 2026-09-23); what is missing is the runtime: either the plugin fetches/installs it on first start, or the runtime ships as its own package the plugin depends on. Also decides how people outside the private repo get it — [[OpenClaw plugin#Ways to distribute it]] |
