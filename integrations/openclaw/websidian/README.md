...
Plain ESM JavaScript, no build step, no dependencies. Tested against OpenClaw 2026.6.9.
...
OpenClaw 2026.6.9's Control UI has no plugin-owned tab, so the pages are reached by URL (the `/brain` command prints
it). Newer OpenClaw releases add native plugin pages (`host.ui.registerPage`, behind the "Custom plugin UI" lab);
that is backlog work.

## Is it really installed?

| # | Check |
|---|---|
| 1 | `openclaw plugins inspect websidian --runtime` lists 4 hooks, tool `websidian_links`, command `brain`, service `websidian-runtime`, 1 HTTP route |
