...
- **In the Hermes tab, a flowing note does not highlight the table of contents as you scroll**: the page scrolls the dashboard, not the frame, so the note never sees a scroll. Clicking a heading still works. The graph and the editor keep a fixed-height frame of their own ([[Hermes plugin#Dashboard tab]]).
- **The agent panel in the Hermes tab is Review only, and Hermes's Review is by instruction**: Hermes cannot be locked read-only from its command line, so a file it changes anyway is only flagged afterwards (with Revert) — on a read-only vault too. Editing with an agent stays in the editor ([[Hermes plugin#Asking an agent]]).
- **The Hermes tab's agents need signing in where the dashboard runs** — inside `hermes01` for the container, not on the Windows host. On 2026-09-23 both `claude` and `codex` there reported "logged in" but their OAuth tokens could not be refreshed (Claude: *OAuth access token has expired*; Codex: *could not be refreshed, sign in again*), and Hermes's own provider (the ChatGPT/Codex subscription) answered *HTTP 429: usage limit reached*. So no agent answered yet in `hermes01`; the panel says which and why — [[Hermes plugin#Asking an agent]].
- **Restarting the dashboard in `hermes01` leaves a zombie behind**: the container's PID 1 is `hermes gateway run`, which does not reap children the dashboard leaves. Harmless; cleared by a container restart.
- **The editor in the Hermes tab keeps its own light chrome** (brand, site switch, toolbar); reading uses the dashboard's colours ([[Improvements backlog]] A21).
- **Hermes tab search shares one budget** — 60 searches a minute across all dashboard users, and each keystroke pause searches every vault ([[Improvements backlog]] A22).
...
- **`openclaw plugins update websidian` does nothing for a tarball install.** An `npm-pack:` install has no registry to ask; install the newer tarball with `--force`. Publishing to npm or ClawHub would fix it (backlog A12).
