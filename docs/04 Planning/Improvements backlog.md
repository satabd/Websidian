...
| A12 | 🟡 2026-09-23: **published on ClawHub** as `websidian@0.2.0` and installed by name ([[OpenClaw plugin#Installing it for someone else]]). Still open: try the ClawHub install in Docker (`alpine/openclaw`) and on Linux/macOS; a CI job (GitHub OIDC trusted publisher, `clawhub package trusted-publisher`) so releases publish from a tag instead of a laptop |
