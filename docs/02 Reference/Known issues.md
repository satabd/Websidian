...
- **One agents test is flaky.** *One session across turns* failed once in a full `npm test` (its first turn came back resumed) and passed on every rerun; some state is shared between test files — [[Agents in the editor]].
